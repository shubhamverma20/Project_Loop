...
export function TestFeedbackCard() {
  ...
  const [result, setResult] = useState<{ success: boolean; message?: string; error?: string; feedback?: { id?: string; category?: string; sentiment?: string } } | null>(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        ...
        setResult({
          success: true,
          message: res.data.message || "Feedback submitted successfully",
          feedback: res.data.data || res.data.feedback
        })
        setContent("")
      } else {
        setResult({ success: false, error: res.data?.error || "Failed to submit feedback" })
      }
    } catch {
      setResult({
        success: false,
