...
export function WorkspaceSettingsForm({ workspace, userRole }: WorkspaceSettingsProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post("/api/settings/workspace", { name })
      if (res.error || !res.data?.success) {
        setError(res.error || res.data?.error || "Failed to update workspace name")
      } else {
        setMessage(res.data?.message || "Workspace updated successfully")
      }
    } catch {
