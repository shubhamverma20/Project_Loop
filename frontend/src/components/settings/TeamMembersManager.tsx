...
export function TeamMembersManager({ members, currentUserId, currentUserRole, onRefresh }: TeamMembersProps) {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post("/api/settings/invite", {
      ...
      if (res.error || !res.data?.success) {
        setError(res.error || res.data?.error || "Failed to invite team member")
      } else {
        setMessage(res.data?.message || "Team member added successfully")
        setInviteEmail("")
        setInviteName("")
  ...
  const handleRoleChange = async (memberId: string, newRole: Role) => {
    ...
    try {
      const res = await api.post("/api/settings/member-role", {
        targetUserId: memberId,
        newRole
      })
      if (res.error || !res.data?.success) {
        setError(res.error || res.data?.error || "Failed to update role")
      } else {
  ...
  const handleRemoveMember = async (memberId: string, memberEmail: string | null) => {
    ...
    try {
      const res = await api.post("/api/settings/remove-member", {
        targetUserId: memberId
      })
      if (res.error || !res.data?.success) {
        setError(res.error || res.data?.error || "Failed to remove member")
      } else {
        setMessage(res.data?.message || "Member removed successfully")
        if (onRefresh) onRefresh()
      }
    } catch {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div>
              ...
              >
                ...
                <option value="ANALYST">ANALYST (Read & Ingest)</option>
